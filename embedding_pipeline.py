...
class ChromaEmbeddingPipelineTextOnly:
    ...
    def add_documents_to_collection(self, documents: List[Tuple[str, Dict[str, Any]]], 
        ...
        if update_mode == 'replace':
            ...
            if file_doc_ids:
                self.collection.delete(ids=file_doc_ids)
                logger.info(f"Deleted {len(file_doc_ids)} existing documents from {file_path.name}")
        ...
        for batch_start in range(0, len(documents), batch_size):
            ...
            # Add batch to collection if there are documents to add
            if batch_ids:
                self.collection.add(
                    ids=batch_ids,
